...
export function rateLimitGuard(
  clientId: string,
  config: RateLimitConfig | boolean,
): void {
  if (config === false) return;
  const cfg =
    config === true ? { windowMs: 60000, max: 100, perClient: true } : config;
  ...
  if (timestamps.length >= max) {
    throw new Error(
      `Rate limit exceeded. Maximum ${max} requests per ${windowMs}ms allowed.`,
    );
  }
